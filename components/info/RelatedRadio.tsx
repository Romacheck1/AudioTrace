...
interface RelatedRadioProps {
  radio: Radio[];
}

export default function RelatedRadio({ radio }: RelatedRadioProps) {
  if (radio.length === 0) return null;

  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">Related Radio Stations</h2>
      <div className="space-y-3">
        {radio.map((item) => (
          <a key={item.id} href={`/radio/${item.id}`} className="flex items-center gap-4 p-3 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors group">
