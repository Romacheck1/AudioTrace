...
interface MovieMainSectionProps {
  movie: Movie;
}
    ...
    <div className="flex gap-8 mb-8 flex-col lg:flex-row items-start lg:items-end">
      ...
      <div className="flex-1 flex flex-col justify-between lg:h-64">
        <div>
          ...
          <p className="text-4xl text-red-700 mb-3">{movie.artist}</p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-base text-gray-600 mb-2">
            <span>Duration: {formatDuration(movie.duration_ms)}</span>
            {movie.genre && <span>Genre: {movie.genre}</span>}
            <span>Rating: {formatRating(movie.popularity)}</span>
          </div>
