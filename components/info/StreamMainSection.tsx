...
interface StreamMainSectionProps {
  stream: Stream;
}

export default function StreamMainSection({ stream }: StreamMainSectionProps) {
  // Format popularity as viewer count
  ...
  return (
    <div className="flex gap-8 mb-8 flex-col lg:flex-row items-start lg:items-end">
      ...
      <div className="flex-1 flex flex-col justify-between lg:h-64">
        <div>
          <h1 className="text-4xl lg:text-5xl font-bold mb-3">{stream.title}</h1>
          <p className="text-4xl text-red-700 mb-3">{stream.artist}</p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-base text-gray-600 mb-2">
            <span>Viewers: {formatViewers(stream.popularity)}</span>
            {stream.genre && <span>Category: {stream.genre}</span>}
