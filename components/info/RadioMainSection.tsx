...
interface RadioMainSectionProps {
  radio: Radio;
}

export default function RadioMainSection({ radio }: RadioMainSectionProps) {
  const starRating = 4.4;
  const reviewCount = '10k';
  ...
  return (
    <div className="flex gap-8 mb-8 flex-col lg:flex-row items-start lg:items-end">
      <div className="w-full lg:w-64 h-64 bg-gray-300 rounded-lg flex-shrink-0 flex items-center justify-center">
        {radio.image_url ? <img src={radio.image_url} alt={`${radio.title} logo`} className="w-full h-full object-cover rounded-lg" /> : <span className="text-gray-500 text-sm">No Image</span>}
      </div>
      <div className="flex-1 flex flex-col justify-between lg:h-64">
        <div>
          <h1 className="text-4xl lg:text-5xl font-bold mb-3">{radio.title}</h1>
          <p className="text-4xl text-red-700 mb-3">{radio.artist}</p>
        </div>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-base text-gray-600 mb-2">
            {radio.genre && <span>Genre: {radio.genre}</span>}
            <span className="text-purple-600 font-semibold">📻 Live Radio</span>
          </div>
