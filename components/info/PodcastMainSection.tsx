...
export default function PodcastMainSection({ podcast }: PodcastMainSectionProps) {
  ...
  // Format popularity/episodes
  const formatEpisodes = (popularity: number | null) => {
  ...
  return (
    <div className="flex gap-8 mb-8 flex-col lg:flex-row items-start lg:items-end">
      ...
      <div className="flex-1 flex flex-col justify-between lg:h-64">
        <div>
          <h1 className="text-4xl lg:text-5xl font-bold mb-3">{podcast.title}</h1>
          <p className="text-4xl text-red-700 mb-3">{podcast.artist}</p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-base text-gray-600 mb-2">
            <span>Episodes: {formatEpisodes(podcast.popularity)}</span>
            {podcast.genre && <span>Genre: {podcast.genre}</span>}
            {podcast.duration_ms && <span>Avg Duration: {formatDuration(podcast.duration_ms)}</span>}
          </div>
