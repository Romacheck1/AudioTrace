...
export default function RelatedInterviews({ interviews }: RelatedInterviewsProps) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">Related Interview Shows</h2>
      <div className="space-y-3">
        {interviews.map((item) => (
          <a key={item.id} href={`/interview/${item.id}`} className="flex items-center gap-4 p-3 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors group">
