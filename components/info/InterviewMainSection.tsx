...
interface InterviewMainSectionProps {
  interview: Interview;
}

export default function InterviewMainSection({ interview }: InterviewMainSectionProps) {
  ...
  const starRating = 4.6;
  const reviewCount = '18k';
  ...
  return (
    <div className="flex gap-8 mb-8 flex-col lg:flex-row items-start lg:items-end">
      <div className="w-full lg:w-64 h-64 bg-gray-300 rounded-lg flex-shrink-0 flex items-center justify-center">
        {interview.image_url ? <img src={interview.image_url} alt={`${interview.title} logo`} className="w-full h-full object-cover rounded-lg" /> : <span className="text-gray-500 text-sm">No Image</span>}
      </div>
      <div className="flex-1 flex flex-col justify-between lg:h-64">
        <div>
          <h1 className="text-4xl lg:text-5xl font-bold mb-3">{interview.title}</h1>
          <p className="text-4xl text-red-700 mb-3">{interview.artist}</p>
        </div>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-base text-gray-600 mb-2">
            <span>Duration: {formatDuration(interview.duration_ms)}</span>
            {interview.genre && <span>Category: {interview.genre}</span>}
            <span className="text-orange-600 font-semibold">🎤 Interview</span>
          </div>
